<section class="section2" id="pre-boda">
  <div class="section2__img">
    <img src="<?php echo IMG_BASE . 'img-2.webp'?>" alt="">
  </div>
  <div class="box">
    <p class="heading--24 text--center">PRE-BODA</p>
    <div class="space space--20"></div>
    <p class="heading--76 text--center">Welcome Cocktail</p>
    <div class="space space--20"></div>
    <p class="heading--24 text--center">JUEVES, 27 DE FEBRERO 2025</p>
    <div class="space space--10"></div>
    <p class="heading--24 text--center">Bar boticario</p>
    <p class="heading--24 text--center">hotel SOFITEL Santa Clara</p>
    <div class="space space--10"></div>
    <p class="heading--24 text--center">5:30 P.M.</p>
    <div class="space space--10"></div>
    <a href="https://maps.app.goo.gl/Zezei67AQy4ZSah5A" class="button button--primary button--center" target="_blank">VER MAPA</a>
    <div class="space space--54"></div>
    <p class="heading--54 text--center">Dress Code</p>
    <div class="space space--10"></div>
    <a href="#" class="button button--secondary button--secondary-small button--center">COCKTAIL</a>
  </div>

  <div class="space space--54"></div>
  <div class="space space--54 desktop"></div>

  <div class="section2__text">
    <p class="heading--96 text--center">R.S.V.P.</p>
    <div class="space space--20"></div>
    <p class="heading--24 text--center">¡CELEBRA CON NOSOTROS!</p>
    <div class="space space--10"></div>
    <p class="heading--20 text--center">Por favor confirmar asistencia antes del 15 de DICIEMBRE, 2025</p>
    <div class="space space--20"></div>
    <a href="#" class="button button--primary button--center">IR AHORA</a>
    
  </div>
</section>